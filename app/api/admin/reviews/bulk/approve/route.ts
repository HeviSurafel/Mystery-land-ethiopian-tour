...
export async function POST(req: NextRequest) {
  try {
    ...
    // Get all reviews to update before changing status (for rating updates)
    ...
    // Update ratings for each unique item
    ...
    return NextResponse.json(
      { success: false, error: error.message || 'Failed to approve reviews' },
      { status: 500 }
    );
  }
}
